...
# Dash APP
import dash
from dash import html, dcc, dash_table as dt, Input, Output, State, no_update
...
df_confirmed_t.index = pd.to_datetime(df_confirmed_t.index, format="%m/%d/%y")
df_deaths_t.index = pd.to_datetime(df_confirmed_t.index, format="%m/%d/%y")
df_recovered_t.index = pd.to_datetime(df_confirmed_t.index, format="%m/%d/%y")

# Highest 10 plot data preprocessing
# getting highest 10 countries with confirmed case
# print([col.split("|")[1] for col in df_confirmed_t.columns])
name = [col.split("|")[1] for col in df_confirmed_t.columns]
...
name = [col.split("|")[1] for col in df_deaths_t.columns]
...
map_data["Recovered"] = (
    map_data["Recovered"].fillna(0).astype(int)
...
map_data["text"] = [
    ...
    for i, j, k, l, m, k24, l24, m24 in zip(
...
def draw_global_graph(
    ...
    df_confirmed_total.index = pd.to_datetime(
        df_confirmed_total.index, format="%m/%d/%y"
    )
...
def draw_singleCountry_Scatter(
    df_confirmed_t, df_deaths_t, df_recovered_t, selected_row=0, daily_change=False
):
...
def draw_singleCountry_Bar(
    df_confirmed_t, df_deaths_t, df_recovered_t, selected_row=0, graph_line="Line Chart"
):
...
def navbar(
    logo="/assets/logo-placeholder.png", height="35px", appname="PlaceHolder Name"
):
    ...
    navbar = dbc.Navbar(
                ...
                [
                    dbc.Col(
                        html.A(
                            ...
                            html.Div(
                                "trich.ai",
                                className="trich-navbar white font-xl",
                            ),
                            # align="center",
                            # no_gutters=True,
                            href="https://trich.ai",
                    ...
                    dbc.Col(
                        dbc.NavbarBrand(appname, className="font-sm text-white"),
        ...
        color="#393939",
        className="bottom32",  # className="bottom16",
...
def display_main_stats():
            ...
            [
                ...
                html.Div(
                    ...
                    + str(
                        round(
                            (
                                (df_confirmed_total[-1] - df_confirmed_total[-2])
    ...
    total_deaths = dbc.Col(
        html.Div(
            [
                html.Div(
                    children="Total Deaths: ",
                    className="text-center text-death bold font-md bottom16",
                ),
                html.Div(
                ...
                html.Div(
                    "Mortality Rate: "
                    + str(round(df_deaths_total[-1] / df_confirmed_total[-1] * 100, 3))
                    + "%",
    ...
    total_recovered = dbc.Col(
        html.Div(
            [
                html.Div(
                    children="Total Recovered: ",
                    className="text-center text-recov bold font-md bottom16",
                ),
                html.Div(
                ...
                html.Div(
                    "Recovery Rate: "
                    + str(
                        round(df_recovered_total[-1] / df_confirmed_total[-1] * 100, 3)
                    )
                    + "%",
    ...
    display = dbc.Row(
        [total_cases, total_deaths, total_recovered], className="bottom32"
    )

    return display


title_application_display = html.Div(
    children="COVID-19 Global Cases",
            ...
            [
                dbc.Col(
                    dcc.RadioItems(
                        ...
                        value="Total Cases",
                        labelStyle={"display": "inline-block", "padding": "0 12px"},
                        style={
                            "fontSize": 18,
                        },
                    ),
...
def map_scatter_corona(data, zoom, lat_focus, long_focus, title):
    ...
    fig.add_trace(
        go.Scattermapbox(
            ...
            marker=go.scattermapbox.Marker(
                size=np.log1p(data["Confirmed"]), color="rgb(255, 0, 0)", opacity=0.7
            ),
    ...
    fig.add_trace(
        go.Scattermapbox(
            ...
            mode="markers",
            marker=go.scattermapbox.Marker(
                size=8, color="rgb(242, 177, 172)", opacity=0.7
            ),
            hoverinfo="none",
        )
    )
    ...
    fig.update_layout(
        ...
        mapbox=dict(
            accesstoken=mapbox_access_token,
            bearing=0,
            center=dict(lat=lat_focus, lon=long_focus),
            pitch=1,
            zoom=zoom,
            style="dark",
        ),
        ...
        dbc.Col(
            [
                html.Div(
                    [
                        html.Div(
                            "Highest cases: ", className="text-white font-sm bottom8"
                        ),
                        html.Div(" + past 24hrs", className="text-conf bold font-xs"),
                    ],
            ...
            [
                html.Div(
                    [
                        html.Div(
                            "Highest mortality: ",
                            className="text-white font-sm bottom8",
                        ),
                        html.Div(
...
def main_text_structure():
    ...
    header = dbc.Col(
        html.Div(
            children="Covid-19 (Coronavirus) Interactive Outbreak Tracker",
            className="text-white dark left font-lg bottom16",
        ),
        width=12,
    )
    days_calc = dbc.Col(
        [
            ...
            html.Span(
                datatime_convert(df_confirmed.columns[-1], 1) + "  00:01 (UTC).",
                className="text-white font-sm",
            ),
        ],
        width=12,
    ...
    outbreak_days = dbc.Col(
        [
            html.Span("Outbreak since 22-Jan-2020: ", className="text-white font-xs"),
            html.Span(
                str(return_outbreakdays(datatime_convert(df_confirmed.columns[-1], 1)))
                + "  days.",
...
inputs = dbc.CardGroup(
    [
        html.Div(
            "Select a Country", style={"textAlign": "center", "marginRight": "16px"}
        ),
        dcc.Dropdown(
            id="country",
            options=[{"label": x, "value": x} for x in data.countrylist],
            value="World",
            style={"color": "black", "minWidth": "275px"},
        ),
    ],
    ...
    [
        dbc.Col(
            [
                dcc.RadioItems(
                    id="graph-type",
                    options=[
                        {"label": i, "value": i} for i in ["Total Cases", "Daily Cases"]
                    ],
                    value="Total Cases",
                    labelStyle={"display": "inline-block", "padding": "0 12px"},
                    style={
                ...
                dcc.Loading(dcc.Graph(id="global-graph"), type="graph"),
            ],
            width=12,
            sm=12,
            lg=6,
        ...
        dbc.Col(
            [
                ...
                dcc.Loading(dcc.Graph(id="high10-graph"), type="graph"),
            ],
            width=12,
            sm=12,
            lg=6,
...
map_charts_group = dbc.Row([dbc.Col([dcc.Graph(id="map-graph-group")], width=10)])
            ...
            [
                dt.DataTable(
                    ...
                    style_cell_conditional=[
                        {"if": {"column_id": "Province/State"}, "width": "26%"},
...
dropdowns = dbc.Row(
    [
        ...
        dbc.Col(
            dcc.Dropdown(placeholder="Select the Province", id="dropdown-province"),
            width=4,
        ),
    ]
)
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    ...
                    children=[
                        dbc.Col(
                            html.Div("Forecast 30 days from today"),
            ...
            style={"marginBottom": "64px", "minHeight": "400px"},
        ),
    ]
)
...
@app.callback(
    Output("main_div", "children"), [Input("tabs-styled-with-inline", "value")]
)
...
def map_selection(data, selected_rows):
    ...
    if len(selected_rows) == 0:
        ...
        return (
            map_scatter_corona(
                aux, zoom, temp_df["Lat"].iloc[0], temp_df["Long"].iloc[0], title="bla"
            ),
            fig1,
            fig2,
        )
...
@app.callback(
    Output("dropdown-province", "options"), [Input("dropdown-country", "value")]
)
def map_selection(dropdown_country):

    var_province = [
        {"label": i, "value": i}
        for i in map_data[map_data["Country/Region"] == dropdown_country][
            "Province/State"
        ].unique()
    ]
    # print(var_province)
...
@app.callback(Output("map-graph", "figure"), [Input("dropdown-country", "value")])
def map_selection(dropdown_country):

    zoom = 1
    if len(dropdown_country) is None:
        return map_scatter_corona(
            map_data, 1, 15, 40, title="Corona Virus Around the world"
        )
    else:
        return map_scatter_corona(
            map_data,
            2,
            map_data[map_data["Country/Region"] == dropdown_country]["Long"].iloc[0],
            map_data[map_data["Country/Region"] == dropdown_country]["Long"].iloc[0],
            title="Corona Virus Around the world",
        )
...
@app.callback(
    output=Output("plot-active", "figure"), inputs=[Input("country", "value")]
)
...
@app.callback(
    output=Output("output-panel", "children"), inputs=[Input("country", "value")]
)
        ...
        [
            html.Div(
                [
                    html.Div(
                        f"Data for {country}",
                        className="text-white font-md width-full bottom32 top16",
                    ...
                    html.Div(
                        [
                            html.Div(
                                [
                                    html.Div(
                                        "Total cases until today:",
                                        className="text-white font-sm width-full bottom8",
                                    ),
                                    html.Div(
                                        "{:,.0f}".format(total_cases_until_today),
                            ...
                            html.Div(
                                [
                                    html.Div(
                                        "Total cases in 30 days:",
                                        className="text-danger font-sm width-full bottom8",
                                    ),
                                    html.Div(
                                        "{:,.0f}".format(total_cases_in_30days),
                            ...
                            html.Div(
                                [
                                    html.Div(
                                        "Active cases in 30 days:",
                                        className="text-danger font-sm width-full bottom8",
                                    ),
                                    html.Div(
                                        "{:,.0f}".format(active_cases_in_30days),
                                        className="text-danger font-md width-full bottom16",
                                        style={"textAlign": "center"}
                                    ),
                                ]
                            ),
                        ], className="dispInfoForecast"
                    ),
                    html.Div(
                        [
                            html.Div(
                            ...
                            html.Div(
                                "with {:,.0f} cases".format(num_max),
                                className="font-sm bottom16",
                                style={"color": 0},
                            ),
                        ], style={"textAlign":"center"}
                    ),
